...
num_best_features = 2 #set to 60 to capture 90% of variance 29 for 75% variance, 10 for 50% of variance
#test_size = .2866
test_size = .3630
...
labels = kmeans.predict(F) # careful with these "labels" very different from "label" above
# Centroid values
...
plt.figure(3)
plt.scatter(f1, f2, c= y) #color them according to cluster (labels) or histology (Label)
plt.scatter(C_x, C_y, marker='*', s=200, c='g')
plt.title('Histology color coded with Centroids (k=4)')
...
cm = confusion_matrix(y, labels)  # first variable Label is vertical, second (labels) is horizontal
